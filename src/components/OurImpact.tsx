          ...
          <ol className="grid grid-cols-1 lg:grid-cols-2 text-center gap-4 w-90 mt-6 max-lg:grid-cols-2">
            {impact.map((details, index) => (
              <div key={index}>
                ...
                <span className="inline-block mt-4 text-[13px]">{details.body}</span>
              </div>
            ))}
          </ol>
    ...
    <>
     <div className="w-full relative bg-gradient-to-b from-[#0A2540] to-[#051829] h-100 lg:h-80 ">
      ...
      <section className="text-center flex flex-col justify-center items-center justify-self-center text-white gap-4 w-90 lg:w-130 py-10 h-full">
        ...
        <span className="text-[12px] max-md:text-[10px]">Join leading airlines transforming post-booking experience</span>
      </section>
